...
Start
 ^BGP router identifier ${BGP_ID}, local AS number ${LOCAL_AS_NUM}\s*$$ 
 ^Neighbor.*State.PfxRcd.*$$ -> IPBgpTable
 
IPBgpTable
  ^${NEIGHBOR}\s+4\s+${REMOTE_AS}\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+${UP_DOWN}\s+${STATE_PFXRCD} -> Record
...
# 6. Parse the following 'show ip bgp summary' output (see link below). From this output, extract the following fields: Neighbor,
# Remote AS, Up_Down, and State_PrefixRcvd. Also include the Local AS and the BGP Router ID in each row of the tabular output (hint:
# use filldown for this). Note, in order to simplify this problem only worry about the data shown in the output (in other words,
